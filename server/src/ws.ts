    ...
    return {
      onMessage(evt, ws) {
        ...
        if (msg.type === 'set_game_mode') {
          if (playerId !== room.adminId) { send(ws, { type: 'error', message: 'Only the host can change the game mode.' }); return }
          if (!VALID_MODES.has(msg.mode)) { send(ws, { type: 'error', message: 'Invalid game mode.' }); return }
          if (room.players.size > MAX_PLAYERS[msg.mode]) { send(ws, { type: 'error', message: `Cannot switch to this mode: too many players (max ${MAX_PLAYERS[msg.mode]}).` }); return }
        ...
        if (msg.type === 'kick_player') {
          if (playerId !== room.adminId) { send(ws, { type: 'error', message: 'Only the host can kick players.' }); return }
          if (msg.playerId === playerId) { send(ws, { type: 'error', message: 'You cannot kick yourself.' }); return }
          const target = room.players.get(msg.playerId)
          if (!target) { send(ws, { type: 'error', message: 'Player not found.' }); return }
        ...
        if (msg.type === 'start_game') {
          if (playerId !== room.adminId) { send(ws, { type: 'error', message: 'Only the host can start the game.' }); return }
          if (room.players.size < 2) { send(ws, { type: 'error', message: 'Need at least 2 players to start.' }); return }
          if (room.gameState !== null) { send(ws, { type: 'error', message: 'Game already started.' }); return }
          const players = [...room.players.values()].map((p) => ({ id: p.id, name: p.name }))
        ...
        if (!room.gameState) { send(ws, { type: 'error', message: 'Game has not started yet.' }); return }

        if (msg.type === 'set_face_up') {
          if (room.gameState.phase !== 'setup') { send(ws, { type: 'error', message: 'Not in setup phase.' }); return }
          ...
          try { room.gameState = setFaceUp(room.gameState, playerId, msg.cardIds) } catch (e) { send(ws, { type: 'error', message: (e as Error).message }); return }
        ...
        if (msg.type === 'play_card') {
          if (room.gameState.phase !== 'playing') { send(ws, { type: 'error', message: 'Game is not in playing phase.' }); return }
          let result
        ...
        if (msg.type === 'pick_up_pile') {
          if (room.gameState.phase !== 'playing') { send(ws, { type: 'error', message: 'Game is not in playing phase.' }); return }
          try { room.gameState = pickUpPile(room.gameState, playerId) } catch (e) { send(ws, { type: 'error', message: (e as Error).message }); return }
          broadcastGameState(room)
          return
        }

        if (msg.type === 'peek_face_down') {
          if (room.gameState.phase !== 'playing') { send(ws, { type: 'error', message: 'Not in playing phase.' }); return }
          const gameState = room.gameState
          const player = gameState.players[gameState.currentPlayerIndex]
          if (player.id !== playerId) { send(ws, { type: 'error', message: 'Not your turn.' }); return }
          ...
          if (!isInFaceDownPhase) { send(ws, { type: 'error', message: 'Not in face-down phase.' }); return }
          const fdMatch = msg.fdId.match(/^fd_(\d+)$/)
          if (!fdMatch) { send(ws, { type: 'error', message: 'Invalid face-down card ID.' }); return }
          const fdIdx = parseInt(fdMatch[1], 10)
          if (fdIdx < 0 || fdIdx >= player.faceDown.length) { send(ws, { type: 'error', message: 'Invalid face-down card index.' }); return }
          send(ws, { type: 'face_down_revealed', fdId: msg.fdId, card: player.faceDown[fdIdx] })
          return
        }

        if (msg.type === 'return_to_lobby') {
          if (!room.gameState || room.gameState.phase !== 'finished') { send(ws, { type: 'error', message: 'Can only return to lobby after the game has finished.' }); return }
          room.gameState = null
